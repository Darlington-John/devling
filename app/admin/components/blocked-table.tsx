...
import BlockedRow from './blocked-row/blocked';
import { blocked_type } from '~/types/blocked';

interface blocked {
	pagedBlocked: blocked_type[] | null;
	pageSize: number;
	searchTerm: string;
...
const BlockedTable = ({
	...
	const hasBlocked = pagedBlocked && pagedBlocked.length > 0;
			...
			<section className="flex gap-3 flex-col w-full  overflow-auto justify-end">
				<div className="flex flex-col   rounded-md  border   border-grey w-full min-w-[1000px]">
					<div className="w-full flex gap-1 bg-grey text-silver">
						...
						<div className="w-[15%] py-2 px-3 text-sm">Date</div>
					...
					>
						{hasBlocked ? (
							<>
								{pagedBlocked.map((blocked) => (
									<BlockedRow key={blocked?._id} blocked={blocked} />
								))}
								{Array(Math.max(0, pageSize - pagedBlocked.length))
									.fill(null)
									.map((_, i) => (
										<div
											key={i}
							...
							<EmptyState
								message={
									hasSearch ? 'No member matches your search' : 'No Blocked yet'
								}
							/>
						)}
					</Loader>
