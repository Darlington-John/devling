...
const EditTopicPrompt = ({
	...
	useEffect(() => {
		if (topicToEdit) {
			...
			setDesc(topicToEdit.description || '');
			setImagePreview(topicToEdit?.image || '');
			setImageUrl(topicToEdit.image || '');
		}
	}, [topicToEdit]);
	const editTopic = async () => {
		...
		formData.append('topic', topicToEdit?._id);
		await apiRequest({
			url: '/api/topics/edit-topic',
			method: 'PATCH',
			body: formData,
			onSuccess: (response) => {
				setSuccessful(true);
...
export default EditTopicPrompt;
